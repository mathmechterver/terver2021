#!/bin/sh
jupyter nbconvert --to html --template classic prac01/prac01.ipynb
jupyter nbconvert --to html --template classic prac02/prac02.ipynb
jupyter nbconvert --to html --template classic prac03/prac03.ipynb
jupyter nbconvert --to html --template classic prac04/prac04.ipynb
jupyter nbconvert --to html --template classic prac05/prac05.ipynb
jupyter nbconvert --to html --template classic prac06/prac06.ipynb

